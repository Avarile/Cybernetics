...
import { CONTACT_CATEGORY_VOCAB_CONFIG } from './contacts.helpers';

@SubCommand({ name: 'ls', description: 'List contact categories' })
export class ContactsCategoryLsCommand extends CommandRunner {
  constructor(
  ...
  async run(_params: string[], options: VocabLsOptions): Promise<void> {
    await runVocabularyLs(CONTACT_CATEGORY_VOCAB_CONFIG, this.settings, this.clients, options);
  }
}

@SubCommand({ name: 'add', description: 'Create a contact category' })
export class ContactsCategoryAddCommand extends CommandRunner {
  constructor(
    private readonly settings: SettingsService,
    private readonly clients: ClientFactory,
  ...
  async run(_params: string[], options: VocabAddOptions): Promise<void> {
    await runVocabularyAdd(CONTACT_CATEGORY_VOCAB_CONFIG, this.settings, this.clients, this.editor, options);
  }
}

@SubCommand({ name: 'edit', arguments: '<id>', description: 'Edit a contact category' })
export class ContactsCategoryEditCommand extends CommandRunner {
  constructor(
    private readonly settings: SettingsService,
    private readonly clients: ClientFactory,
    @Optional() private readonly editor: EditorService = new EditorService(),
  ...
  async run(params: string[], options: VocabCommonOptions): Promise<void> {
    const [id] = params;
    await runVocabularyEdit(CONTACT_CATEGORY_VOCAB_CONFIG, this.settings, this.clients, this.editor, id, options);
  }
}

@SubCommand({ name: 'rm', arguments: '<id>', description: 'Delete a contact category' })
export class ContactsCategoryRmCommand extends CommandRunner {
  constructor(
    private readonly settings: SettingsService,
    private readonly clients: ClientFactory,
    @Optional()
...
export class ContactsCategoryCommand extends CommandRunner {
