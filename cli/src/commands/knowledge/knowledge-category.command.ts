...
import { KNOWLEDGE_CATEGORY_VOCAB_CONFIG } from './knowledge.helpers';

@SubCommand({ name: 'ls', description: 'List knowledge categories' })
export class KnowledgeCategoryLsCommand extends CommandRunner {
  constructor(
  ...
  async run(_params: string[], options: VocabLsOptions): Promise<void> {
    await runVocabularyLs(KNOWLEDGE_CATEGORY_VOCAB_CONFIG, this.settings, this.clients, options);
  }
}

@SubCommand({ name: 'add', description: 'Create a knowledge category' })
export class KnowledgeCategoryAddCommand extends CommandRunner {
  constructor(
    private readonly settings: SettingsService,
    private readonly clients: ClientFactory,
  ...
  async run(_params: string[], options: VocabAddOptions): Promise<void> {
    await runVocabularyAdd(KNOWLEDGE_CATEGORY_VOCAB_CONFIG, this.settings, this.clients, this.editor, options);
  }
}

@SubCommand({ name: 'edit', arguments: '<id>', description: 'Edit a knowledge category' })
export class KnowledgeCategoryEditCommand extends CommandRunner {
  constructor(
    private readonly settings: SettingsService,
    private readonly clients: ClientFactory,
    @Optional() private readonly editor: EditorService = new EditorService(),
  ...
  async run(params: string[], options: VocabCommonOptions): Promise<void> {
    const [id] = params;
    await runVocabularyEdit(KNOWLEDGE_CATEGORY_VOCAB_CONFIG, this.settings, this.clients, this.editor, id, options);
  }
}

@SubCommand({ name: 'rm', arguments: '<id>', description: 'Delete a knowledge category' })
export class KnowledgeCategoryRmCommand extends CommandRunner {
  constructor(
    private readonly settings: SettingsService,
    private readonly clients: ClientFactory,
    @Optional()
  ...
  async run(params: string[], options: VocabRmOptions): Promise<void> {
    ...
    await runVocabularyRm(
      KNOWLEDGE_CATEGORY_VOCAB_CONFIG,
      this.settings,
      this.clients,
      this.confirmPrompt,
      this.isTTY,
...
@SubCommand({
  name: 'category',
  description: 'Manage knowledge categories',
  subCommands: [
...
export class KnowledgeCategoryCommand extends CommandRunner {
