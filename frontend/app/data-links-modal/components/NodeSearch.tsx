...
interface Props {
  ...
  onClear?: () => void;
  /** When this changes to a non-null node, imperatively commit that selection */
  imperativeSelect?: SearchNode | null;
}
...
export function NodeSearch({ nodes, onSelect, onClear, imperativeSelect }: Props) {
  ...
  const prevImperativeIdRef = useRef<string | null>(null);
  ...
  // Imperative selection driven by parent (mock state)
  useEffect(() => {
    const prevId = prevImperativeIdRef.current;
    const currId = imperativeSelect?.id ?? null;
    prevImperativeIdRef.current = currId;

    if (currId && currId !== prevId) {
      // Commit a new node selection
      setSelected(imperativeSelect!);
      setQuery('');
      setOpen(false);
      onSelect(imperativeSelect!);
    } else if (!currId && prevId !== null) {
      // Reset (e.g. simulation looped)
      setSelected(null);
      setQuery('');
      setOpen(false);
      onClear?.();
